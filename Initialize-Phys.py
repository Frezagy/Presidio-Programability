...
def AEP_build_phys(host, user, profile, domain):
...
def AEP_build_L3(host, user, profile, domain):
	print ('[BEG] Attached Entity Profile Configuration')
	moDir = apic_login(host, user, password)
	...
	print ('	Associating Domain: '+domain+' to AEP: '+aepName)
	infraRsDomP2 = RsDomP(infraAttEntityP, tDn=u'uni/l3dom-'+domain)
	
	cfg_commit(moDir,infraInfra)
	print ('[END] Attached Entity Profile Configuration')
	return aepName
	
def l3_domain(host, user, password):

	moDir = apic_login(host, user, password)
	
	polUni = Uni('')

	l3DomName='L3-OUT-L3-DOMAIN-Cobra'
	vpool='RoutingVLAN'
	l3extDomP = l3ext.DomP(polUni, name=l3DomName)
	infraRsVlanNs = RsVlanNs(l3extDomP, tDn=u'uni/infra/vlanns-['+vpool+']-static')
	
	cfg_commit(moDir, l3extDomP)
...
def tenant_policy(host, user, password):
	...
	Ctx(fvTenantMo, "DC_INSIDE")
	Ctx(fvTenantMo, "DC_OUTISDE")
	...
	fvSubnet = Subnet(fvBDMo2, name=u'Sub3', ip=u'106.0.3.1/24', preferred=u'no', virtual=u'no')
	fvSubnet = Subnet(fvBDMo2, name=u'Sub4', ip=u'106.0.4.1/24', preferred=u'no', virtual=u'no')
	fvSubnet = Subnet(fvBDMo2, name=u'Sub5', ip=u'106.0.5.1/24', preferred=u'no', virtual=u'no')

	# Create association to private network
	RsCtx(fvBDMo1, tnFvCtxName="DC_INSIDE")
	RsCtx(fvBDMo2, tnFvCtxName="DC_OUTSIDE")

	# Create Application Profile
	fvApMo = Ap(fvTenantMo, 'ASA')

	fvAEPg1 = AEPg(fvApMo, 'WEB_SERVERS')
	fvAEPgBD1 = RsBd(fvAEPg1, tnFvBDName=u'SERVER_BD1')
	...
	fvAEPgBD1= RsBd(fvAEPg2, tnFvBDName='SERVER_BD2')
...
if __name__ == '__main__':
	...
	""""Testing L3 - Leave AssocDomain = l3_domain(host, user, password)"""
	
	#aepName = AEP_build_phys(host, user, password, AssocDomain)
